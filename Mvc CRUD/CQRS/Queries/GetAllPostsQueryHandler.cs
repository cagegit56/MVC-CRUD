...
internal sealed class GetAllPostsQueryHandler : IRequestHandler<GetAllPostsQuery, PaginateResponse<List<PostsDto>>>
{
    ...
    public async Task<PaginateResponse<List<PostsDto>>> Handle(GetAllPostsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!newUser.success)
            {
                _logger.LogError($"Unable to add a user due to {newUser.error}");
        ...
        catch (Exception ex)
        {
            ...
            return new PaginateResponse<List<PostsDto>>() { Error = "Failed to retrieve posts" };
        }
    }
}
